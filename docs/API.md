...
2. **FastAPI v2 Routes:** Controlled via `slowapi` (`Limiter`) initialized in `API/main.py`. Rate limiting is applied at `25 per minute` per IP to the `GET /api/v2/weather` route.
